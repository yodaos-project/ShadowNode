...
static void finalize(napi_env env, void* finalize_data, void* finalize_hint) {
  ...
  if (*f_hint != native_hint)
    napi_fatal_error(__FILE__, NAPI_AUTO_LENGTH, "finalize hint not aligned.", NAPI_AUTO_LENGTH);
  *f_data += 1;
}

static void cleanup(void* data) {
  if (native_counter == 0) {
    napi_fatal_error(__FILE__, NAPI_AUTO_LENGTH, "finalize not invoked.", NAPI_AUTO_LENGTH);
  }
}
...
napi_value Wrap(napi_env env, napi_callback_info info) {
  ...
  napi_ref weak_ref;
  NAPI_CALL(env, napi_wrap(env, argv[0], &native_counter, finalize, &native_hint, &weak_ref));
  /**
