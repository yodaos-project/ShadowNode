...
static JNativeInfoType dbus_module_native_info = { .free_cb = NULL };
static JNativeInfoType this_module_native_info = {
  .free_cb = (jerry_object_native_free_callback_t)iotjs_dbus_destroy
};
...
static bool iotjs_dbus_encode_jobject(jerry_value_t val, DBusMessageIter* iter,
  ...
  switch (type) {
    ...
    case DBUS_TYPE_SIGNATURE: {
      ...
      jerry_string_to_utf8_char_buffer(val, jerry_buffer, size);
      buffer[size] = '\0';

      dbus_bool_t r = dbus_message_iter_append_basic(iter, type, &buffer);
...
static void iotjs_dbus_call_method(DBusPendingCall* pending, void* data) {
  ...
  if (!msg) {
    jerry_release_value(method_data->jcallback);
    return;
  }
