...
static void iotjs_mqtt_alloc_buf(unsigned char **buf,
                                 int expected_size,
                                 int *alloc_size) {
...
JS_FUNCTION(MqttDeserialize) {
  ...
  iotjs_string_t topic_str = iotjs_string_create_with_size(topic.lenstring.data,
                                                           topic_len);
  ...
  iotjs_bufferwrap_copy(payload_wrap,
                        (const char*)payload,
                        (size_t)payload_len);
